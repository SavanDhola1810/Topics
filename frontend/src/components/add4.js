...
const Add2 = () => {
    ...
    const { subContent4, addSubContent3, addContentOfText4, getSubContent3, editSubContent3 } = c;

    const { id4, subTopic4 } = useParams();
    ...
    const handleSubmit1 = (e) => {
        e.preventDefault()
        if (text.length > 3) {
            addContentOfText4(id4.slice(1), subTopic4.slice(1), text, 'text', false)
            setText('')
            getSubContent3()
    ...
    const handleSubmit2 = (e) => {
        ...
        if ((link.slice(0, 5)) === 'https') {
            addSubContent3(id4.slice(1), subTopic4.slice(1), link, 'link')
            setLink('')
            getSubContent3()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const handleSubmit3 = (e) => {
        ...
        if (paragraph.length > 10) {
            addSubContent3(id4.slice(1), subTopic4.slice(1), paragraph, 'paragraph')
            setParagraph('')
            getSubContent3()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const buttonClick = () => {
        ...
        paragraphRef.current.style.display = "none"
        textRef.current.style.display = "none"
        blurRef.current.style.filter="blur(9px)"
    ...
    const handleSort = async () => {
        ...
        // getContent()
        await editSubContent3(dragItem.current, dragOveritem.current)
    ...
    return (
        <div className='w-full images hello' style={{
            ...
            <div className=' text-4xl p-4 text-center border-gray-600 ' style={{ boxShadow: "0px 6px 8px black",backgroundColor:"rgba(20,20,78,0.3)",fontWeight: "500",zIndex:"1", bottomBorderColorOpacity:"0.5",color: "whitesmoke" ,borderBottomWidth:"3px",borderBottomColor:""}}>{subTopic4.slice(1)}</div>
              ...
              <div className=" w-4/5 mx-auto" style={{
               ...
               >
                ...
                <p className="m-10 text-lg  text-justify flex flex-col "  >
                    {subContent4 && subContent4.filter((ele, ind) => {
                        if (ele) {
                            console.log("hello ddjjsj")
                            return ele?.topicID == (id4.slice(1))
                        }
                    }).map((item, index) => (
                        <p style={{ whiteSpace: "pre-line" }} draggable key={index}
        ...
        {isVisible && <p className="cursor-pointer fixed" style={{ top: "80%", right: "3.5%" }}>
          <span onClick={goToTop} className='p-3 border-3  ' style={{ borderColor: "#3399ff", color: "", borderRadius: "50%" }}>
            ...
            <div className="z-2  grid  h-full w-full place-items-center " style={{ overflowX: "hidden", color: "rgb(226,232,240)" }} >
                <div className='hidden fixed top-1/4  flex-col  p-4 h-auto border-2 border-slate-400 w-1/4  rounded-2xl ' style={{ backgroundColor: "rgb(63,63,73)" }} ref={modalRef}>
