...
const Add = () => {
  ...
  const { content1, addContent, addContentOfText, editClickable, getContent, editContent, subContent1, getSubContent } = c;
  const { id, topic, status } = useParams();
  ...
  const handleSubmit1 = (e) => {
    e.preventDefault()
    if (text.length > 3) {
      addContentOfText(id.slice(1), topic.slice(1), text, 'text', state)
      setText('')
      getContent()
  ...
  const handleSubmit2 = (e) => {
    ...
    if ((link.slice(0, 5)) === 'https') {
      addContent(id.slice(1), topic.slice(1), link, 'link')
      setLink('')
      getContent()
      modalRef.current.style.display = "none"
      blurRef.current.style.filter="blur(0px)"
  ...
  const handleSubmit3 = (e) => {
    ...
    if (paragraph.length > 10) {
      addContent(id.slice(1), topic.slice(1), paragraph, 'paragraph')
      setParagraph('')
      getContent()
      modalRef.current.style.display = "none"
      blurRef.current.style.filter="blur(0px)"
  ...
  const handleSort =async  () => {
    ...
    await editContent(dragItem.current, dragOveritem.current)
  ...
  // console.log(count)
  const countFun = (item, item1) => {
    let count = 0
    subContent1.map((ele, ind) => {
      if (ele.topicID == item) {
        count = count + 1
      }
      else {
  ...
  const editC = async (id) => {
    ...
    const response = await fetch(`${host}/api/content/editClickable`, {
      method: "POST",
      headers: {
  ...
  return (
    <div className='w-full images hello' style={{
      ...
      <div className=' text-4xl p-4 text-center border-gray-600 ' style={{ boxShadow: "0px 6px 8px black",backgroundColor:"rgba(20,20,78,0.3)",fontWeight: "500",zIndex:"1", bottomBorderColorOpacity:"0.5",color: "whitesmoke" ,borderBottomWidth:"3px",borderBottomColor:""}}>{topic.slice(1)}</div>
      ...
      blurRef.current.style.filter="blur(0px)"}}>
        <div className=" w-4/5 mx-auto" style={{
         ...
         >
          ...
          <p className="m-10 text-lg tracking-wide  text-justify flex flex-col " >
            {content1.length != 0 && content1.filter((ele, inde) => {
              if (ele) {
                return ele?.topicID == id.slice(1)
              }
              {/* rgb(59 64 91) */}
            }).map((item, index) => {
              return (
        ...
        <div className="z-2  grid  h-full w-full place-items-center " style={{ overflowX: "hidden", color: "rgb(226,232,240)" }} >
          <div className='hidden fixed top-1/4  flex-col  p-4 h-auto border-2 border-gray-400 w-1/4  rounded-2xl ' style={{ backgroundColor: "rgb(63,63,73)" }} ref={modalRef}>
            ...
            <input type="submit" value="Submit" ref={submit1Ref} className=" hidden justify-center mt-4 -mb-4 text-white bg-sky-400  p-2 w-full rounded-xl text-xl cursor-pointer hover:bg-sky-500" onClick={handleSubmit1} style={{ textAlign: "center" }} />
            ...
            <input type="submit" value="Submit" ref={submit3Ref} className=" hidden justify-center mt-4  text-white bg-sky-400  p-2 w-full rounded-xl text-xl cursor-pointer hover:bg-sky-500" onClick={handleSubmit3} style={{ textAlign: "center" }} />
