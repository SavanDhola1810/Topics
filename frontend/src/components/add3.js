...
const Add2 = () => {
    ...
    const { subContent3, addSubContent2, getSubContent2, addContentOfText3, editSubContent2, subContent4, getSubContent3 } = c;

    const { id3, subTopic3 } = useParams();
    ...
    const handleSubmit1 = (e) => {
        e.preventDefault()
        if (text.length > 3) {
            addContentOfText3(id3.slice(1), subTopic3.slice(1), text, 'text', false)
            setText('')
            getSubContent2()
    ...
    const handleSubmit2 = (e) => {
        ...
        if ((link.slice(0, 5)) === 'https') {
            addSubContent2(id3.slice(1), subTopic3.slice(1), link, 'link')
            setLink('')
            getSubContent2()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const handleSubmit3 = (e) => {
        ...
        if (paragraph.length > 10) {
            addSubContent2(id3.slice(1), subTopic3.slice(1), paragraph, 'paragraph')
            setParagraph('')
            getSubContent2()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const handleSort = async () => {
        ...
        // getContent()
        await editSubContent2(dragItem.current, dragOveritem.current)
    ...
    const editC = async (id) => {
        ...
        const response = await fetch(`${host}/api/subContent2/editClickable`, {
            method: "POST",
            headers: {
    ...
    return (
        <div className='w-full images hello' style={{
            ...
            <div className=' text-4xl p-4 text-center border-gray-600 ' style={{ boxShadow: "0px 6px 8px black",backgroundColor:"rgba(20,20,78,0.3)",fontWeight: "500",zIndex:"1", bottomBorderColorOpacity:"0.5",color: "whitesmoke" ,borderBottomWidth:"3px",borderBottomColor:""}}>{subTopic3.slice(1)}</div>
            ...
            blurRef.current.style.filter="blur(0px)"}}>
              ...
              <p className="m-10 text-lg  text-justify flex flex-col " >
                    {subContent3 && subContent3.filter((ele, ind) => {
                        if (ele) {
                            return ele?.topicID == (id3.slice(1))
                        }
                    }).map((item, index) => {
                        return (
                            <p key={index} >
