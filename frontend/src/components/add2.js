...
const Add2 = () => {
    ...
    const { subContent2, addSubContent1, addContentOfText2, getSubContent1, editSubContent1, subContent3, getSubContent2 } = c;

    const { id2, subTopic2 } = useParams();
    ...
    const handleSubmit1 = (e) => {
        e.preventDefault()
        if (text.length > 3) {
            addContentOfText2(id2.slice(1), subTopic2.slice(1), text, 'text', false)
            setText('')
            getSubContent1()
    ...
    const handleSubmit2 = (e) => {
        ...
        if ((link.slice(0, 5)) === 'https') {
            addSubContent1(id2.slice(1), subTopic2.slice(1), link, 'link')
            setLink('')
            getSubContent1()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const handleSubmit3 = (e) => {
        ...
        if (paragraph.length > 10) {
            addSubContent1(id2.slice(1), subTopic2.slice(1), paragraph, 'paragraph')
            setParagraph('')
            getSubContent1()
            modalRef.current.style.display = "none"
            blurRef.current.style.filter="blur(0px)"
    ...
    const handleSort = async () => {
        ...
        // getContent()
        await editSubContent1(dragItem.current, dragOveritem.current)
    ...
    const editC = async (id) => {
        ...
        const response = await fetch(`${host}/api/subContent1/editClickable`, {
            method: "POST",
            headers: {
    ...
    return (
        <div className='w-full images hello' style={{
            ...
            <div className=' text-4xl p-4 text-center border-gray-600 ' style={{ boxShadow: "0px 6px 8px black",backgroundColor:"rgba(20,20,78,0.3",fontWeight: "500",zIndex:"1", bottomBorderColorOpacity:"0.5",color: "whitesmoke" ,borderBottomWidth:"3px",borderBottomColor:""}}>{subTopic2.slice(1)}</div>
            ...
            blurRef.current.style.filter="blur(0px)"}}>
              ...
              <p className="m-10 text-lg  text-justify flex flex-col "  >
                    {subContent2 && subContent2.filter((ele, ind) => {
                        if (ele) {
                            return ele?.topicID == (id2.slice(1))
                        }
                    }).map((item, index) => {
                        return (
                            <p key={index} >
